...
```mermaid
flowchart TD
    A[句子里需要出现第二个动词] --> B{主句动词是情态动词吗？<br>können, müssen, wollen等}
    B -->|是| C[不需要 zu，原形动词直接踢到句末]
    B -->|否| D{主句动词是感觉/运动动词吗？<br>sehen, hören, gehen等}
    D -->|是| C
    D -->|否| E[必须戴上 zu 这个访客牌！]
    E --> F{这个原形动词是可分动词吗？}
    F -->|否| G[zu + 动词原形<br>例如：zu machen]
    F -->|是| H[夹心饼干结构：前缀 + zu + 动词核心<br>例如：aufzumachen]
```
...
graph LR
    ...
    C --> F["Ich habe vor, mich um die Stelle <b>zu bewerben</b>."]
