...
**狙击位 2：精准否定某个特定成分 —— Nicht 放在该成分的正前方**
...
为了保证你在六个月内达到B2，我们需要把语法融入到实战中。关于“否定”，你的规划如下：

- **第1-2周 (B1基础巩固)**：只练习 `Kein` 的第四格、第三格词尾变化（这是极易扣分点），并习惯把 `Nicht` 放在句末或动词前面。每天用德语描述3件你“没有”的东西和你“不做”的事。
- **第3-4周 (B1-B2过渡)**：刻意练习 `Nicht` 在“框形结构”（完成时、情态动词）中的位置。
- **第2-6个月 (B2突破)**：在写作和口语中强制自己使用 `Niemand`, `Nichts`, `Weder...noch`。B2考试的考官非常看重你是否能用高级连接词来表达否定的逻辑。

---

### 📝 大师实战考核 (Next Step)

理论讲完了，现在的舞台交给你！想象你正在德国办理落户登记（Anmeldung），请尝试将以下三句话翻译成德语。如果你能准确翻译，说明你已经完全掌握了今天的精髓：

1. **不**，我**没有**带租房合同。(提示：独立否定 + 零数量否定)
2. 这**不是**我的护照。(提示：特定归属名词的否定)
3. 我明天**不能**来拿签证。(提示：情态动词框形结构中的否定)

**你要不要现在试着翻译一下这三句话，让我来为你做个专业的批改和精讲？**
