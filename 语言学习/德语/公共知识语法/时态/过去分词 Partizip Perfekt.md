...
```mermaid
graph TD
    A[拿到一个动词原形 <br> e.g. kaufen, verstehen, einkaufen] --> B{它是以 -ieren 结尾吗?}
    B -- 是 <br> (如 telefonieren) --> C[👑 海归派: 砍掉en加t, 坚决不要ge-]
    C --> C1(结果: telefoniert)
    
    B -- 否 --> D{它带有前缀吗?}
    
    D -- 带不可分前缀 <br> (如 ver-, be-, er-) --> E[🔒 死忠粉: 霸占位置, 不要ge-]
    E --> E1(结果: verkauft, verstanden)
    
    D -- 带可分前缀 <br> (如 ein-, auf-, aus-) --> F[💔 异地恋: 把ge-像第三者一样夹在中间]
    F --> F1(结果: eingekauft, aufgestanden)
    
    D -- 无前缀 <br> (如 kaufen, stehen) --> G{它是乖宝宝还是叛逆者?}
    
    G -- 乖宝宝 <br> (规则/弱变化) --> H[🍔 标准汉堡: ge- + 词干 + t]
    H --> H1(结果: gekauft)
    
    G -- 叛逆者 <br> (不规则/强变化) --> I[🍔 变异汉堡: ge- + 变异词干 + en]
    I --> I1(结果: gestanden)

    classDef standard fill:#e1f5fe,stroke:#01579b,stroke-width:2px;
    classDef result fill:#dcedc8,stroke:#33691e,stroke-width:2px;
    class A,B,D,G standard;
    class C1,E1,F1,H1,I1 result;
```
