...
^5hw0ae
![[image-355.png|1090]]
这群“VIP 大佬”非常娇气，它们只要一离开家门（离开第一格/主语的安全区），不论去哪（变成第二、三、四格），都必须要带上它们的“专属保镖”——**词尾 `-n` 或 `-en` **。
...
---
[[语言学习/德语/公共知识语法/名词.md#^1fLAuIG0|100%]]


## 场景和定义

别和名词的复数变化搞混了。用到弱变化名词的时候，就像它的名字一样，它的变化是很微弱的，不用像其他复数或格一样分 -er、-en。你只需要记住，用到了弱变化名词，如果它不在第一格的位置上充当主语，那么它只需要加 -en 就够了

|格|强变化 der Hund|弱变化 der Student|
|---|---|---|
|单数第一格|der Hund|der Student|
|单数第四格|den Hund|den Studenten|
|单数第三格|dem Hund|dem Studenten|
|单数第二格|des Hundes|des Studenten|

使用场合：这个名词只要不在单数第一格的位置上，就加 -(e)n。 没有别的条件——不看句型、不看语体、不看介词。

Der Student fragt den Kollegen.
这个大学生问那位同事。

`Der Student` 单数第一格 → 光杆；`den Kollegen` 单数第四格 → 加 -n。同一句里两个弱变化名词，形式不同只因为位置不同。

背单词时把它当性别的一部分记：`der Student, -en, -en`（第二格词尾、复数词尾），看到两个 -en 就知道是弱变化。

## 识别
**只有一小部分特定的阳性名词**属于“阳性弱变化名词”（德语中称为 **N-Deklination**）。

绝大多数普通的阳性名词（比如 der Mann, der Tisch）在第四格和第三格时，名词本身是不发生变化的（只有前面的冠词变），仅在第二格时加 -s 或 -es。
#### 1. 以 `-e` 结尾的雄性动物 -n
...
#### 2. 外来词 `-and`, `-ant`, `-ent`, `-at`, `-ist` `-graf` (主要指人) -en
