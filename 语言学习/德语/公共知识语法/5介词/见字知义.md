...
### 第三章：你的半年征服计划书（嵌入 B 1-B 2 学习路径）

要达到 B 2 水平，并熟练运用这些介词，不能孤立学习，而要融入学习阶段。

**阶段一：夯实基础期（第 1-2 个月）**
*   **重心**：巩固所有常用介词（an, auf, in, mit, zu...）及其支配的格，特别是 **静三动四**和动词固定搭配。这是你使用高级介词的地基。
*   **接触**：在阅读 **B 1 级别的官方短文**（如简单的房屋规定、市政通知）时，开始**圈出**这些“见字知义”的介词。不要求会用，但要求能通过拆词法猜出意思。先从 `mithilfe` 和 `im Falle` 入手，它们最接近生活。

**阶段二：系统输入期（第 3-4 个月）**
*   **重心**：系统学习 **B 2 语法**，如被动语态、从句化简。这正是咱们今天这些高级介词大展拳脚的地方！
    *   `Zwecks` = `um ... zu + Infinitiv`
    *   `Im Falle` + Gen. = `Wenn`
    *   `Mithilfe` = `indem` 从句
*   **行动**：本周学完“被动语态”，马上造句：“维修工作**借助**特殊工具进行。” —— `Die Reparatur wird mithilfe eines Spezialwerkzeugs durchgeführt.` 这就是“语法联动”。

**阶段三：模拟实战期（第 5 个月）**
*   **重心**：B 2 考试写作（如正式投诉信、求职信）和口语第二部分（观点阐述）专项训练。
*   **行动**：
    *   **投诉信**：必用 `entgegen der Absprache` (与约定相反)， `seitens des Vermieters` (房东方面)， `zwecks Mängelbehebung` (为了修复缺陷)。
    *   **观点阐述**：必用 `im Gegensatz zu` 进行对比，用 `zugunsten der Umwelt` (有利于环境) 来支撑论点。
    *   把这个单子上剩下的词（`anhand`, `zuliebe`）作为你的“杀手锏词汇”，每个月集中火力攻克一个战队。

**阶段四：冲刺与内化期（第 6 个月）**
*   **重心**：套题训练，让表达成为肌肉记忆。
*   **行动**：每次写作练习，规定自己必须使用至少 3 个今天学的介词。对照答案后，检查用法是否正确（特别是格！）。
*   **心态**：你已经不是在“背单词”，而是在“调用一个精准的句式工具”，就像选出合适的螺丝刀。
