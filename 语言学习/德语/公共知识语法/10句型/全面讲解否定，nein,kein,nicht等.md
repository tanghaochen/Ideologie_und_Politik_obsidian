...
```mermaid
flowchart TD
    A[你想在句子里否定什么？] --> B{是在回答一个\n完整的Yes/No问题吗？}
    B -- "是 (Yes)" --> C["Nein (独立保镖)"]
    B -- "否 (No)" --> D{你要否定的是一个名词吗？}
    D -- "否 (动词/形容词/副词等)" --> E["Nicht (精准狙击手)"]
    D -- "是 (Yes)" --> F{这个名词带什么冠词？}
    F -- "不定冠词 (ein/eine)\n或 无冠词 (零冠词)" --> G["Kein (库存管理员)"]
    F -- "定冠词 (der/die/das)\n或 物主代词 (mein/dein)" --> H["Nicht (精准狙击手)"]
```
...
```mermaid
flowchart TD
    Root([德语高级否定词]) --> Time{时间上的黑洞}
    Root --> Entity{人与物的真空}
    Root --> State{状态的改变}
    Root --> Condition{条件的剥夺}
    Root --> Space{空间的盲区}

    Time --> T1["nie / niemals (从不，绝不)"]
    
    Entity --> E1["niemand (没有人)"]
    Entity --> E2["nichts (什么都没有)"]
    
    State --> S1["nicht mehr (不再)"]
    State --> S2["kein... mehr (不再有...)"]
    
    Condition --> C1["ohne + Akk. (没有...)"]
    
    Space --> P1["nirgendwo (哪儿都不)"]
```
...
```mermaid
flowchart TD
    Root([ohne 的三次进化]) --> LevelA[基础版 A1/A2]
    Root --> LevelB1[进阶版 B1]
    Root --> LevelB2[终极版 B2]


    LevelA -->|剥夺物品/人| Prep["ohne + 名词/代词 (Akk.)"]
    Prep --> Ex1["Ohne Termin (没有预约) geht nichts."]

    LevelB1 -->|剥夺主语自己的动作| Inf["ohne ... zu + 动词原形"]
    Inf --> Rule1["⚠️ 核心铁律：主句和从句的主语必须是同一个人！"]
    Rule1 --> Ex2["Ich gehe, ohne zu zahlen. (我没付钱就走了)"]

    LevelB2 -->|剥夺别人预期该做的动作| Dass["ohne dass + 句子"]
    Dass --> Rule2["⚠️ 核心铁律：主句和从句的主语不是同一个人！"]
    Rule2 --> Ex3["Er geht, ohne dass ich es merke. (他走了，我都没发觉)"]
```
...
```mermaid
flowchart LR
    Root([德语否定双胞胎]) --> Nicht["nicht (不 / 没有)"]
    Root --> Nichts["nichts (什么都没有)"]

    Nicht -->|词性| Adverb[副词]
    Nicht -->|反义词| Schon[schon / doch]
    Nicht -->|核心功能| Action[否定动作、状态、特征或整个句子]
    Nicht -->|大师类比| Eraser[像一块橡皮擦：擦掉某个原本成立的动作或形容词]

    Nichts -->|词性| Pronoun[不定代词]
    Nichts -->|反义词| Etwas[etwas / alles]
    Nichts -->|核心功能| Object[绝对地否定事物本身，等同于英文的 nothing]
    Nichts -->|大师类比| EmptyBox[像一个空盒子：打开一看，里面空空如也]
```
...
| un/in 与 los 区别                          | un 是否定<br>los 是没有                                                                                                                                                          |                                                                                                                                                                                     |
