...
在讲解读法之前，我们必须先跨越一个文化和书写习惯的鸿沟。

> **大师避坑指南：** 在中文和英文里，我们用“点（.）”来表示小数，比如 **7.4**。 但在德语（以及大部分欧洲语言）中，**小数是用“逗号（,）”来表示的！** 德语中的“点（.）”反而是用来作为千位分隔符的。

- 中文的 **7.4** = 德语的 **7,4**
- 中文的 **1,000** （一千） = 德语的 **1.000**

因此，德语里的小数点不读作 Punkt（点），而是读作 **Komma（逗号）**。

发音公式：

[整数的正常读法] + Komma + [小数部分逐个念出数字]

- 7,4 -> sieben Komma vier （7 点 4）
- 0,5 -> null Komma fünf （0 点 5）
- 3,14 -> drei Komma eins vier （3 点 14。注意：绝对不能读成 drei Komma vierzehn！）
- 12,987 -> zwölf Komma neun acht sieben （12 点 987）

## 第几

^rg78cv

#### 📌 规则 A：1 到 19 的数字（后缀添加 `-te`）

对于 1 到 19 的数字，我们通常直接在词干后面加上 `-te`。但语言总是充满人性化的“不完美”，在这个区间里，有 4 个数字比较调皮，属于**不规则变化**，你必须把它们像朋友的名字一样死死记住：

- **第 1**：**erste** _(极度特殊，必须背！不是 einte)_
- 第 2：zweite _(规则)_
- **第 3**：**dritte** _(极度特殊，必须背！不是 dreite)_
- 第 4：vierte _(规则)_
- 第 5：fünfte _(规则)_
- 第 6：sechste _(规则)_
- **第 7**：**siebte** _(微调：去掉了基数词 sieben 里的 en)_
- **第 8**：**achte** _(微调：因为 acht 已经以 t 结尾，所以只加了 e，避免发音吃力)_

#### 📌 规则 B：第 20 及以上的数字（后缀添加 `-ste`）

一旦数字到达了 20，德语就像是突然换了一个流水线标准，所有的序数词后缀一律变成 ** `-ste` **。这里**没有任何不规则特例**，极其舒适！

- **第几十**系列：
    - 第 20：zwanzig**ste**
    - 第 30：dreißig**ste**
    - 第 40：vierzig**ste**
- **混合数字**（如第 21，第 45）：只在最后一个单词上加后缀！
    - 第 21：einundzwanzig**ste** (个位数的 ein 保持不变，只有最后的 zwanzig 加了 ste)
    - 第 45：fünfundvierzig**ste**
- **百千万**系列：
    - **第 100**：hundert**ste**
    - 第 1000：tausend**ste**
    - 第 1000000：million**ste** (注意百万是加-ste)
