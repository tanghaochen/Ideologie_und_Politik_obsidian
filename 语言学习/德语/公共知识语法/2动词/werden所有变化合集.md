...
```mermaid
graph TD
    A[werden 核心动词] -->|1. 身份是实意/系动词<br/>表示: 变得、成为| B(完成时态 Perfekt / Plusquamperfekt)
    A -->|2. 身份是被动语态助动词<br/>表示: 被...| C(完成时态的被动语态)
    
    B --> D[保留完整前缀 ge- <br/> 过去分词形态: geworden]
    D --> E(例: Er ist krank geworden. 他病了)
    
    C --> F[强制脱落前缀 ge- <br/> 过去分词形态: worden]
    F --> G(为何脱落？避免连续发两个 ge- 的音，显得极其拗口)
    G --> H(例: Das Haus ist gebaut worden. 房子已被建好)
```
