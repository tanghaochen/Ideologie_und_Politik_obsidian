...
```mermaid
graph TD
    A((动词: lassen)) --> B(用法一：实打实地留下<br>Nicht mitnehmen)
    A --> C(用法二：放任与允许<br>Erlauben / nicht erlauben)
    A --> D(用法三：找人代劳<br>Nicht selbst machen)

    B --> B1[作为普通实义动词使用]
    B1 --> B2[完成时: hat ... gelassen<br>使用正常的第二分词]
    B2 --> B3[例: Ich habe den Koffer hier gelassen.]

    C --> C1[作为情态动词的近亲使用<br>lassen + 另一个动词原形]
    C1 --> C2[⚠️ 核心考点 ⚠️<br>完成时: hat ... 动词原形 + lassen<br>这就是所谓的'双不定式' Ersatzinfinitiv]
    C2 --> C3[例: Ich habe ihn gehen lassen.]

    D --> C1
    D --> D1[⚠️ 核心考点 ⚠️<br>完成时同样使用'双不定式']
    D1 --> D2[例: Ich habe das Auto reparieren lassen.]
    
    style A fill:#ff9999,stroke:#333,stroke-width:4px
    style C2 fill:#ffeb3b,stroke:#333,stroke-width:2px,color:#000
