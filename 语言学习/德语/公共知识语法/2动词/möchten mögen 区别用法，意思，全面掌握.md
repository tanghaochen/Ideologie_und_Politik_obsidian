...
flowchart TD
    ...
    B -->|长期偏好、性格习惯、纯粹的喜欢| C(使用 mögen)
    B -->|当下的愿望、具体的需要、礼貌的请求| D(使用 möchten)
    
    C --> E[通常只接名词 Akkusativ]
    C -.->|错误陷阱| F[不要加动词！喜欢做某事请用 gern]
    E --> G[例子: Ich mag das Wetter in München.]
    
    D --> H[可以接名词 Akkusativ]
    D --> I[可以接动词原形 放在句末]
    H --> J[例子: Ich möchte die Speisekarte, bitte.]
    I --> K[例子: Ich möchte ein Konto eröffnen.]
```
