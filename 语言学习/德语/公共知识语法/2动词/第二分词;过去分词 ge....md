...
```mermaid
flowchart TD
    A["不可分动词 (untrennbare Verben) <br> 例如带前缀 be-, ver-, er-"] --> B{"是否需要加 'ge-' 前缀？"}
    B -->|"绝对不加！前缀已霸占首位"| C{"判断动词本身的强弱类型"}
    C -->|"弱变化动词 (schwache Verben)"| D["不可分前缀 + 词干 + (e)t"]
    C -->|"强变化动词 (starke Verben)"| E["不可分前缀 + (变音)词干 + en"]
    D --> F["例：vermieten -> vermietet (出租)"]
    D --> G["例：erklären -> erklärt (解释)"]
    E --> H["例：verstehen -> verstanden (理解)"]
    E --> I["例：bekommen -> bekommen (获得)"]
```

我们把不可分动词分为“弱变化”和“强变化”两类，结合您在德国可能遇到的实际场景来进行操练。

###### 1. 弱变化不可分动词 (Schwache untrennbare Verben)

- **公式：不可分前缀 + 动词词干 + t (或 et)**
- **解析：** 动词词干完全不变，只是词尾按照弱变化规则加上 **-t**。如果词干以 d, t 结尾，为了发音方便加上 **-et**。

**【场景一：租房事务】**

- **bezahlen (支付):** be + zahl + t = **bezahlt**
    - _例句:_ Ich habe die Miete für diesen Monat schon **bezahlt**.
    - _翻译:_ 我已经支付了哪怕这个月的房租。
- **vermieten (出租):** ver + miet + et = **vermietet**
    - _例句:_ Der Vermieter hat die Wohnung leider schon an eine andere Familie **vermietet**.
    - _翻译:_ 房东很遗憾地把这套公寓租给另一个家庭了。

**【场景二：行政办事】**

- **erklären (解释):** er + klär + t = **erklärt**
    - _例句:_ Die Beamtin im Ausländeramt hat mir den Prozess genau **erklärt**.
    - _翻译:_ 外管局的女公务员向我详细解释了这个流程。

###### 2. 强变化不可分动词 (Starke untrennbare Verben)

- **公式：不可分前缀 + (可能发生元音变化的)动词词干 + en**
- **解析：** 词尾保持 **-en** 不变，但中间的动词词干可能会像变形金刚一样发生元音变化（这就需要您像记英语不规则动词一样去积累了）。

**【场景三：找工作与职场】**

- **bekommen (获得/得到):** be + komm + en = **bekommen** (词干无变化)
    - _例句:_ Ich habe gestern endlich meine Aufenthaltserlaubnis und den neuen Arbeitsvertrag **bekommen**.
    - _翻译:_ 我昨天终于拿到了我的居留许可和新的工作合同。
- **unterschreiben (签署 - 在此作不可分前缀):** unter + schieb (变音为 schrieb) + en = **unterschrieben**
    - _例句:_ Haben Sie den Arbeitsvertrag schon **unterschrieben**?
    - _翻译:_ 您已经签了那份工作合同了吗？

**【场景四：医疗就诊】**

- **verstehen (理解):** ver + steh (变音为 stand) + en = **verstanden**
    - _例句:_ Der Arzt hat mir die Diagnose mitgeteilt, aber ich habe wegen der Fachbegriffe nicht alles **verstanden**.
    - _翻译:_ 医生告知了我诊断结果，但因为专业术语，我并没有完全听懂。
- **empfehlen (推荐):** emp + fehl (变音为 fohl) + en = **empfohlen**
    - _例句:_ Mein Hausarzt hat mir dieses Medikament **empfohlen**.
    - _翻译:_ 我的家庭医生给我推荐了这款药。
