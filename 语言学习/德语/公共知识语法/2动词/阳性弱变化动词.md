...
组装家具或者做事 **“machen” (做/干)** 为例，来看看它的“二分词（Partizip II）”是怎么在流水线上加工出来的：

```mermaid
graph LR
A["第一步: 动词原形: machen"] --> B["第二步: 砍掉尾巴 -en"]
B --> C("第三步: 核心词干: mach")
C --> D{"第四步: 弱变化流水线"}
D -->|"头上统一戴帽子"| E["ge-"]
D -->|"脚上统一穿鞋子"| F["-t"]
E --> G["第五步: 最终成品: gemacht"]
F --> G
```
...
特例：它们**不需要戴 `ge-` 这顶帽子**，只需要穿 `-t` 这双鞋子

#### 1：外来词： 以 `-ieren` 结尾的动词  去 en -> t

外来词 它们都以 `-ieren` 结尾， 去 en -> t
